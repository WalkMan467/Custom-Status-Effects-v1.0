...
$execute \
    if score @s cse.status_effects.armor.id = @n[tag=cse.status_effect.armor.data,tag=cse.status_effect.armor.id.$(id),tag=cse.status_effect.armor.$(type),distance=0..,type=marker] cse.status_effects.armor.id run \
tag @s remove cse.status_effect.armor.new

$attribute @s[tag=cse.status_effect.armor.new] minecraft:armor modifier add cse.status_effects.armor.$(id).$(type) $(base) $(type)

tag @s remove cse.status_effect.armor.new

# Detect if there are duplicate IDs
$execute \
    unless entity @n[tag=cse.status_effect.armor.data,tag=cse.status_effect.armor.id.$(id),tag=cse.status_effect.armor.$(type),distance=0..,type=marker] run \
tag @s add cse.status_effect.armor.new

# Call function using the result
$execute \
    if entity @s[tag=cse.status_effect.armor.new] run \
function cse:status_effects/apply/armor/marker_data/add/1 {id:"$(id)", duration:$(duration), base:$(base), value:$(value), max:$(max), type:"$(type)"}
...
$execute \
    ...
    if entity @n[tag=cse.status_effect.armor.data,tag=cse.status_effect.armor.id.$(id),tag=cse.status_effect.armor.$(type),distance=0..,type=marker] run \
function cse:status_effects/apply/armor/marker_data/modify {id:"$(id)", duration:$(duration), value:$(value), max:$(max), type:"$(type)"}
