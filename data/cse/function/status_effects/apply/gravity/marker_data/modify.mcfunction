...
$execute \
    as @n[sort=arbitrary,tag=cse.status_effect.gravity.id.$(id),tag=cse.status_effect.gravity.$(type),tag=cse.status_effect.gravity.data_target,tag=cse.status_effect.gravity.data,distance=0..,type=marker] run \
...
$data modify storage cse:status_effects gravity.final_value.type set value "$(type)"
...
data modify storage temp2 temp set from storage cse:status_effects gravity.final_value
