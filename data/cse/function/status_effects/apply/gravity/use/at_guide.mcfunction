...
$execute \
    if score @s cse.status_effects.gravity.id = @n[tag=cse.status_effect.gravity.data,tag=cse.status_effect.gravity.id.$(id),distance=0..,type=marker] cse.status_effects.gravity.id run \
tag @s remove cse.status_effect.gravity.new

$attribute @s[tag=cse.status_effect.gravity.new] minecraft:gravity modifier add cse.status_effects.gravity.$(id) $(base) add_multiplied_base

tag @s remove cse.status_effect.gravity.new

# Detect if there are duplicate IDs
$execute \
    unless entity @n[tag=cse.status_effect.gravity.data,tag=cse.status_effect.gravity.id.$(id),distance=0..,type=marker] run \
tag @s add cse.status_effect.gravity.new

# Call function using the result
$execute \
    if entity @s[tag=cse.status_effect.gravity.new] run \
function cse:status_effects/apply/gravity/marker_data/add/1 {id:"$(id)", duration:$(duration), base:$(base), value:$(value), max:$(max)}
...
$execute \
    ...
    if entity @n[tag=cse.status_effect.gravity.data,tag=cse.status_effect.gravity.id.$(id),distance=0..,type=marker] run \
function cse:status_effects/apply/gravity/marker_data/modify {id:"$(id)", duration:$(duration), value:$(value), max:$(max)}
