...
$execute \
    as @n[sort=arbitrary,tag=cse.status_effect.armor_toughness.id.$(id),tag=cse.status_effect.armor_toughness.$(type),tag=cse.status_effect.armor_toughness.data_target,tag=cse.status_effect.armor_toughness.data,distance=0..,type=marker] run \
function cse:status_effects/apply/armor_toughness/marker_data/math {duration:$(duration),value:$(value), max:$(max)}

$attribute @s minecraft:armor_toughness modifier remove cse.status_effects.armor_toughness.$(id).$(type)
...
$data modify storage cse:status_effects armor_toughness.final_value.type set value "$(type)"
...
data modify storage temp2 temp set from storage cse:status_effects armor_toughness.final_value
