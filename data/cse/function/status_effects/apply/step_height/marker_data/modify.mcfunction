...
$execute \
    as @n[sort=arbitrary,tag=cse.status_effect.step_height.id.$(id),tag=cse.status_effect.step_height.data_target,tag=cse.status_effect.step_height.data,distance=0..,type=marker] run \
function cse:status_effects/apply/step_height/marker_data/math {duration:$(duration),value:$(value), max:$(max)}

$attribute @s minecraft:step_height modifier remove cse.status_effects.step_height.$(id)
