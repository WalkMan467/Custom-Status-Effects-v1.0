...
$execute \
    if score @s cse.status_effects.step_height.id = @n[tag=cse.status_effect.step_height.data,tag=cse.status_effect.step_height.id.$(id),distance=0..,type=marker] cse.status_effects.step_height.id run \
tag @s remove cse.status_effect.step_height.new

$attribute @s[tag=cse.status_effect.step_height.new] minecraft:step_height modifier add cse.status_effects.step_height.$(id) $(base) add_multiplied_base

tag @s remove cse.status_effect.step_height.new

# Detect if there are duplicate IDs
$execute \
    unless entity @n[tag=cse.status_effect.step_height.data,tag=cse.status_effect.step_height.id.$(id),distance=0..,type=marker] run \
tag @s add cse.status_effect.step_height.new

# Call function using the result
$execute \
    if entity @s[tag=cse.status_effect.step_height.new] run \
function cse:status_effects/apply/step_height/marker_data/add/1 {id:"$(id)", duration:$(duration), base:$(base), value:$(value), max:$(max)}
...
$execute \
    ...
    if entity @n[tag=cse.status_effect.step_height.data,tag=cse.status_effect.step_height.id.$(id),distance=0..,type=marker] run \
function cse:status_effects/apply/step_height/marker_data/modify {id:"$(id)", duration:$(duration), value:$(value), max:$(max)}
