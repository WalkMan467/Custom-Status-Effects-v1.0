...
$execute \
    as @n[sort=arbitrary,tag=cse.status_effect.jump_strength.id.$(id),tag=cse.status_effect.jump_strength.data_target,tag=cse.status_effect.jump_strength.data,distance=0..,type=marker] run \
function cse:status_effects/apply/jump_strength/marker_data/math {duration:$(duration),value:$(value), max:$(max)}

$attribute @s minecraft:jump_strength modifier remove cse.status_effects.jump_strength.$(id)
