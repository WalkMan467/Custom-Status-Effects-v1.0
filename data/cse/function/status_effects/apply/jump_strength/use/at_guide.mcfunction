...
$execute \
    if score @s cse.status_effects.jump_strength.id = @n[tag=cse.status_effect.jump_strength.data,tag=cse.status_effect.jump_strength.id.$(id),tag=cse.status_effect.jump_strength.$(type),distance=0..,type=marker] cse.status_effects.jump_strength.id run \
tag @s remove cse.status_effect.jump_strength.new

$attribute @s[tag=cse.status_effect.jump_strength.new] minecraft:jump_strength modifier add cse.status_effects.jump_strength.$(id).$(type) $(base) $(type)

tag @s remove cse.status_effect.jump_strength.new

# Detect if there are duplicate IDs
$execute \
    unless entity @n[tag=cse.status_effect.jump_strength.data,tag=cse.status_effect.jump_strength.id.$(id),tag=cse.status_effect.jump_strength.$(type),distance=0..,type=marker] run \
tag @s add cse.status_effect.jump_strength.new

# Call function using the result
$execute \
    if entity @s[tag=cse.status_effect.jump_strength.new] run \
function cse:status_effects/apply/jump_strength/marker_data/add/1 {id:"$(id)", duration:$(duration), base:$(base), value:$(value), max:$(max), type:"$(type)"}
...
$execute \
    ...
    if entity @n[tag=cse.status_effect.jump_strength.data,tag=cse.status_effect.jump_strength.id.$(id),tag=cse.status_effect.jump_strength.$(type),distance=0..,type=marker] run \
function cse:status_effects/apply/jump_strength/marker_data/modify {id:"$(id)", duration:$(duration), value:$(value), max:$(max), type:"$(type)"}
