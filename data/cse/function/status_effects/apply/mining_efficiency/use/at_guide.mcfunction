...
$execute \
    if score @s cse.status_effects.mining_efficiency.id = @n[tag=cse.status_effect.mining_efficiency.data,tag=cse.status_effect.mining_efficiency.id.$(id),distance=0..,type=marker] cse.status_effects.mining_efficiency.id run \
tag @s remove cse.status_effect.mining_efficiency.new

$attribute @s[tag=cse.status_effect.mining_efficiency.new] minecraft:mining_efficiency modifier add cse.status_effects.mining_efficiency.$(id) $(base) add_multiplied_base

tag @s remove cse.status_effect.mining_efficiency.new

# Detect if there are duplicate IDs
$execute \
    unless entity @n[tag=cse.status_effect.mining_efficiency.data,tag=cse.status_effect.mining_efficiency.id.$(id),distance=0..,type=marker] run \
tag @s add cse.status_effect.mining_efficiency.new

# Call function using the result
$execute \
    if entity @s[tag=cse.status_effect.mining_efficiency.new] run \
function cse:status_effects/apply/mining_efficiency/marker_data/add/1 {id:"$(id)", duration:$(duration), base:$(base), value:$(value), max:$(max)}
...
$execute \
    ...
    if entity @n[tag=cse.status_effect.mining_efficiency.data,tag=cse.status_effect.mining_efficiency.id.$(id),distance=0..,type=marker] run \
function cse:status_effects/apply/mining_efficiency/marker_data/modify {id:"$(id)", duration:$(duration), value:$(value), max:$(max)}
