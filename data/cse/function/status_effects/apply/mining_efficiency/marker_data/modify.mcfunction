...
$execute \
    as @n[sort=arbitrary,tag=cse.status_effect.mining_efficiency.id.$(id),tag=cse.status_effect.mining_efficiency.data_target,tag=cse.status_effect.mining_efficiency.data,distance=0..,type=marker] run \
function cse:status_effects/apply/mining_efficiency/marker_data/math {duration:$(duration),value:$(value), max:$(max)}

$attribute @s minecraft:mining_efficiency modifier remove cse.status_effects.mining_efficiency.$(id)
