...
$execute \
    if score @s cse.status_effects.block_break_speed.id = @n[tag=cse.status_effect.block_break_speed.data,tag=cse.status_effect.block_break_speed.id.$(id),distance=0..,type=marker] cse.status_effects.block_break_speed.id run \
tag @s remove cse.status_effect.block_break_speed.new

$attribute @s[tag=cse.status_effect.block_break_speed.new] minecraft:block_break_speed modifier add cse.status_effects.block_break_speed.$(id) $(base) add_multiplied_base

tag @s remove cse.status_effect.block_break_speed.new

# Detect if there are duplicate IDs
$execute \
    unless entity @n[tag=cse.status_effect.block_break_speed.data,tag=cse.status_effect.block_break_speed.id.$(id),distance=0..,type=marker] run \
tag @s add cse.status_effect.block_break_speed.new

# Call function using the result
$execute \
    if entity @s[tag=cse.status_effect.block_break_speed.new] run \
function cse:status_effects/apply/block_break_speed/marker_data/add/1 {id:"$(id)", duration:$(duration), base:$(base), value:$(value), max:$(max)}
...
$execute \
    ...
    if entity @n[tag=cse.status_effect.block_break_speed.data,tag=cse.status_effect.block_break_speed.id.$(id),distance=0..,type=marker] run \
function cse:status_effects/apply/block_break_speed/marker_data/modify {id:"$(id)", duration:$(duration), value:$(value), max:$(max)}
