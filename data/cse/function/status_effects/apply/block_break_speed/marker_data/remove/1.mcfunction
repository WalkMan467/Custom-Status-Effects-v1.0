...
function cse:status_effects/apply/block_break_speed/marker_data/remove/2 with storage cse:status_effects.id.uuid temp

# Reference the "ID" macro in the function
# And perform data deletion on the player affected by the status
function cse:status_effects/apply/block_break_speed/marker_data/remove/3 with storage cse:status_effects.id.uuid temp

execute \
    if score #reset.block_break_speed cse.global.main matches 1 run \
function cse:status_effects/apply/block_break_speed/marker_data/remove/6