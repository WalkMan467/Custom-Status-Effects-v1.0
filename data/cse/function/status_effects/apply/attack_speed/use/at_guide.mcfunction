...
$execute \
    if score @s cse.status_effects.attack_speed.id = @n[tag=cse.status_effect.attack_speed.data,tag=cse.status_effect.attack_speed.id.$(id),tag=cse.status_effect.attack_speed.$(type),distance=0..,type=marker] cse.status_effects.attack_speed.id run \
tag @s remove cse.status_effect.attack_speed.new

$attribute @s[tag=cse.status_effect.attack_speed.new] minecraft:attack_speed modifier add cse.status_effects.attack_speed.$(id).$(type) $(base) $(type)

tag @s remove cse.status_effect.attack_speed.new

# Detect if there are duplicate IDs
$execute \
    unless entity @n[tag=cse.status_effect.attack_speed.data,tag=cse.status_effect.attack_speed.id.$(id),tag=cse.status_effect.attack_speed.$(type),distance=0..,type=marker] run \
tag @s add cse.status_effect.attack_speed.new

# Call function using the result
$execute \
    if entity @s[tag=cse.status_effect.attack_speed.new] run \
function cse:status_effects/apply/attack_speed/marker_data/add/1 {id:"$(id)", duration:$(duration), base:$(base), value:$(value), max:$(max), type:"$(type)"}
...
$execute \
    ...
    if entity @n[tag=cse.status_effect.attack_speed.data,tag=cse.status_effect.attack_speed.id.$(id),tag=cse.status_effect.attack_speed.$(type),distance=0..,type=marker] run \
function cse:status_effects/apply/attack_speed/marker_data/modify {id:"$(id)", duration:$(duration), value:$(value), max:$(max), type:"$(type)"}
