...
$execute \
    if score @s cse.status_effects.tempt_range.id = @n[tag=cse.status_effect.tempt_range.data,tag=cse.status_effect.tempt_range.id.$(id),tag=cse.status_effect.tempt_range.$(type),distance=0..,type=marker] cse.status_effects.tempt_range.id run \
tag @s remove cse.status_effect.tempt_range.new

$attribute @s[tag=cse.status_effect.tempt_range.new] minecraft:tempt_range modifier add cse.status_effects.tempt_range.$(id).$(type) $(base) $(type)

tag @s remove cse.status_effect.tempt_range.new

# Detect if there are duplicate IDs
$execute \
    unless entity @n[tag=cse.status_effect.tempt_range.data,tag=cse.status_effect.tempt_range.id.$(id),tag=cse.status_effect.tempt_range.$(type),distance=0..,type=marker] run \
tag @s add cse.status_effect.tempt_range.new

# Call function using the result
$execute \
    if entity @s[tag=cse.status_effect.tempt_range.new] run \
function cse:status_effects/apply/tempt_range/marker_data/add/1 {id:"$(id)", duration:$(duration), base:$(base), value:$(value), max:$(max), type:"$(type)"}
...
$execute \
    ...
    if entity @n[tag=cse.status_effect.tempt_range.data,tag=cse.status_effect.tempt_range.id.$(id),tag=cse.status_effect.tempt_range.$(type),distance=0..,type=marker] run \
function cse:status_effects/apply/tempt_range/marker_data/modify {id:"$(id)", duration:$(duration), value:$(value), max:$(max), type:"$(type)"}
