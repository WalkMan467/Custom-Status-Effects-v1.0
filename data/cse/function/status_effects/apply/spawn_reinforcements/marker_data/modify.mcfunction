...
$execute \
    as @n[sort=arbitrary,tag=cse.status_effect.spawn_reinforcements.id.$(id),tag=cse.status_effect.spawn_reinforcements.$(type),tag=cse.status_effect.spawn_reinforcements.data_target,tag=cse.status_effect.spawn_reinforcements.data,distance=0..,type=marker] run \
function cse:status_effects/apply/spawn_reinforcements/marker_data/math {duration:$(duration),value:$(value), max:$(max)}

$attribute @s minecraft:spawn_reinforcements modifier remove cse.status_effects.spawn_reinforcements.$(id).$(type)
...
$data modify storage cse:status_effects spawn_reinforcements.final_value.type set value "$(type)"
...
data modify storage temp2 temp set from storage cse:status_effects spawn_reinforcements.final_value
