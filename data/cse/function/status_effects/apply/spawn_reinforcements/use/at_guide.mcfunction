...
$execute \
    if score @s cse.status_effects.spawn_reinforcements.id = @n[tag=cse.status_effect.spawn_reinforcements.data,tag=cse.status_effect.spawn_reinforcements.id.$(id),distance=0..,type=marker] cse.status_effects.spawn_reinforcements.id run \
tag @s remove cse.status_effect.spawn_reinforcements.new

$attribute @s[tag=cse.status_effect.spawn_reinforcements.new] minecraft:spawn_reinforcements modifier add cse.status_effects.spawn_reinforcements.$(id) $(base) add_multiplied_base

tag @s remove cse.status_effect.spawn_reinforcements.new

# Detect if there are duplicate IDs
$execute \
    unless entity @n[tag=cse.status_effect.spawn_reinforcements.data,tag=cse.status_effect.spawn_reinforcements.id.$(id),distance=0..,type=marker] run \
tag @s add cse.status_effect.spawn_reinforcements.new

# Call function using the result
$execute \
    if entity @s[tag=cse.status_effect.spawn_reinforcements.new] run \
function cse:status_effects/apply/spawn_reinforcements/marker_data/add/1 {id:"$(id)", duration:$(duration), base:$(base), value:$(value), max:$(max)}
...
$execute \
    ...
    if entity @n[tag=cse.status_effect.spawn_reinforcements.data,tag=cse.status_effect.spawn_reinforcements.id.$(id),distance=0..,type=marker] run \
function cse:status_effects/apply/spawn_reinforcements/marker_data/modify {id:"$(id)", duration:$(duration), value:$(value), max:$(max)}
