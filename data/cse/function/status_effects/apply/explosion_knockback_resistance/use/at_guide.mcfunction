...
$execute \
    if score @s cse.status_effects.explosion_knockback_resistance.id = @n[tag=cse.status_effect.explosion_knockback_resistance.data,tag=cse.status_effect.explosion_knockback_resistance.id.$(id),distance=0..,type=marker] cse.status_effects.explosion_knockback_resistance.id run \
tag @s remove cse.status_effect.explosion_knockback_resistance.new

$attribute @s[tag=cse.status_effect.explosion_knockback_resistance.new] minecraft:explosion_knockback_resistance modifier add cse.status_effects.explosion_knockback_resistance.$(id) $(base) add_multiplied_base

tag @s remove cse.status_effect.explosion_knockback_resistance.new

# Detect if there are duplicate IDs
$execute \
    unless entity @n[tag=cse.status_effect.explosion_knockback_resistance.data,tag=cse.status_effect.explosion_knockback_resistance.id.$(id),distance=0..,type=marker] run \
tag @s add cse.status_effect.explosion_knockback_resistance.new

# Call function using the result
$execute \
    if entity @s[tag=cse.status_effect.explosion_knockback_resistance.new] run \
function cse:status_effects/apply/explosion_knockback_resistance/marker_data/add/1 {id:"$(id)", duration:$(duration), base:$(base), value:$(value), max:$(max)}
...
$execute \
    ...
    if entity @n[tag=cse.status_effect.explosion_knockback_resistance.data,tag=cse.status_effect.explosion_knockback_resistance.id.$(id),distance=0..,type=marker] run \
function cse:status_effects/apply/explosion_knockback_resistance/marker_data/modify {id:"$(id)", duration:$(duration), value:$(value), max:$(max)}
