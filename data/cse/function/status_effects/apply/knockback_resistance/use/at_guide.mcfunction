...
$execute \
    if score @s cse.status_effects.knockback_resistance.id = @n[tag=cse.status_effect.knockback_resistance.data,tag=cse.status_effect.knockback_resistance.id.$(id),tag=cse.status_effect.knockback_resistance.$(type),distance=0..,type=marker] cse.status_effects.knockback_resistance.id run \
tag @s remove cse.status_effect.knockback_resistance.new

$attribute @s[tag=cse.status_effect.knockback_resistance.new] minecraft:knockback_resistance modifier add cse.status_effects.knockback_resistance.$(id).$(type) $(base) $(type)

tag @s remove cse.status_effect.knockback_resistance.new

# Detect if there are duplicate IDs
$execute \
    unless entity @n[tag=cse.status_effect.knockback_resistance.data,tag=cse.status_effect.knockback_resistance.id.$(id),tag=cse.status_effect.knockback_resistance.$(type),distance=0..,type=marker] run \
tag @s add cse.status_effect.knockback_resistance.new

# Call function using the result
$execute \
    if entity @s[tag=cse.status_effect.knockback_resistance.new] run \
function cse:status_effects/apply/knockback_resistance/marker_data/add/1 {id:"$(id)", duration:$(duration), base:$(base), value:$(value), max:$(max), type:"$(type)"}
...
$execute \
    ...
    if entity @n[tag=cse.status_effect.knockback_resistance.data,tag=cse.status_effect.knockback_resistance.id.$(id),tag=cse.status_effect.knockback_resistance.$(type),distance=0..,type=marker] run \
function cse:status_effects/apply/knockback_resistance/marker_data/modify {id:"$(id)", duration:$(duration), value:$(value), max:$(max), type:"$(type)"}
