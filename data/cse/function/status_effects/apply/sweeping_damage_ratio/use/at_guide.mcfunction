...
$execute \
    if score @s cse.status_effects.sweeping_damage_ratio.id = @n[tag=cse.status_effect.sweeping_damage_ratio.data,tag=cse.status_effect.sweeping_damage_ratio.id.$(id),tag=cse.status_effect.sweeping_damage_ratio.$(type),distance=0..,type=marker] cse.status_effects.sweeping_damage_ratio.id run \
tag @s remove cse.status_effect.sweeping_damage_ratio.new

$attribute @s[tag=cse.status_effect.sweeping_damage_ratio.new] minecraft:sweeping_damage_ratio modifier add cse.status_effects.sweeping_damage_ratio.$(id).$(type) $(base) $(type)

tag @s remove cse.status_effect.sweeping_damage_ratio.new

# Detect if there are duplicate IDs
$execute \
    unless entity @n[tag=cse.status_effect.sweeping_damage_ratio.data,tag=cse.status_effect.sweeping_damage_ratio.id.$(id),tag=cse.status_effect.sweeping_damage_ratio.$(type),distance=0..,type=marker] run \
tag @s add cse.status_effect.sweeping_damage_ratio.new

# Call function using the result
$execute \
    if entity @s[tag=cse.status_effect.sweeping_damage_ratio.new] run \
function cse:status_effects/apply/sweeping_damage_ratio/marker_data/add/1 {id:"$(id)", duration:$(duration), base:$(base), value:$(value), max:$(max), type:"$(type)"}
...
$execute \
    ...
    if entity @n[tag=cse.status_effect.sweeping_damage_ratio.data,tag=cse.status_effect.sweeping_damage_ratio.id.$(id),tag=cse.status_effect.sweeping_damage_ratio.$(type),distance=0..,type=marker] run \
function cse:status_effects/apply/sweeping_damage_ratio/marker_data/modify {id:"$(id)", duration:$(duration), value:$(value), max:$(max), type:"$(type)"}
