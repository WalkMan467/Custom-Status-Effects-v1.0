...
$execute \
    if score @s cse.status_effects.waypoint_receive_range.id = @n[tag=cse.status_effect.waypoint_receive_range.data,tag=cse.status_effect.waypoint_receive_range.id.$(id),distance=0..,type=marker] cse.status_effects.waypoint_receive_range.id run \
tag @s remove cse.status_effect.waypoint_receive_range.new

$attribute @s[tag=cse.status_effect.waypoint_receive_range.new] minecraft:waypoint_receive_range modifier add cse.status_effects.waypoint_receive_range.$(id) $(base) add_multiplied_base

tag @s remove cse.status_effect.waypoint_receive_range.new

# Detect if there are duplicate IDs
$execute \
    unless entity @n[tag=cse.status_effect.waypoint_receive_range.data,tag=cse.status_effect.waypoint_receive_range.id.$(id),distance=0..,type=marker] run \
tag @s add cse.status_effect.waypoint_receive_range.new

# Call function using the result
$execute \
    if entity @s[tag=cse.status_effect.waypoint_receive_range.new] run \
function cse:status_effects/apply/waypoint_receive_range/marker_data/add/1 {id:"$(id)", duration:$(duration), base:$(base), value:$(value), max:$(max)}
...
$execute \
    ...
    if entity @n[tag=cse.status_effect.waypoint_receive_range.data,tag=cse.status_effect.waypoint_receive_range.id.$(id),distance=0..,type=marker] run \
function cse:status_effects/apply/waypoint_receive_range/marker_data/modify {id:"$(id)", duration:$(duration), value:$(value), max:$(max)}
