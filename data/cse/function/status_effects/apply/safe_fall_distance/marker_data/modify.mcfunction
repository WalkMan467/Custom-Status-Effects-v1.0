...
$execute \
    as @n[sort=arbitrary,tag=cse.status_effect.safe_fall_distance.id.$(id),tag=cse.status_effect.safe_fall_distance.data_target,tag=cse.status_effect.safe_fall_distance.data,distance=0..,type=marker] run \
function cse:status_effects/apply/safe_fall_distance/marker_data/math {duration:$(duration),value:$(value), max:$(max)}

$attribute @s minecraft:safe_fall_distance modifier remove cse.status_effects.safe_fall_distance.$(id)
