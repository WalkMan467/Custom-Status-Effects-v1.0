...
$execute \
    if score @s cse.status_effects.safe_fall_distance.id = @n[tag=cse.status_effect.safe_fall_distance.data,tag=cse.status_effect.safe_fall_distance.id.$(id),distance=0..,type=marker] cse.status_effects.safe_fall_distance.id run \
tag @s remove cse.status_effect.safe_fall_distance.new

$attribute @s[tag=cse.status_effect.safe_fall_distance.new] minecraft:safe_fall_distance modifier add cse.status_effects.safe_fall_distance.$(id) $(base) add_multiplied_base

tag @s remove cse.status_effect.safe_fall_distance.new

# Detect if there are duplicate IDs
$execute \
    unless entity @n[tag=cse.status_effect.safe_fall_distance.data,tag=cse.status_effect.safe_fall_distance.id.$(id),distance=0..,type=marker] run \
tag @s add cse.status_effect.safe_fall_distance.new

# Call function using the result
$execute \
    if entity @s[tag=cse.status_effect.safe_fall_distance.new] run \
function cse:status_effects/apply/safe_fall_distance/marker_data/add/1 {id:"$(id)", duration:$(duration), base:$(base), value:$(value), max:$(max)}
...
$execute \
    ...
    if entity @n[tag=cse.status_effect.safe_fall_distance.data,tag=cse.status_effect.safe_fall_distance.id.$(id),distance=0..,type=marker] run \
function cse:status_effects/apply/safe_fall_distance/marker_data/modify {id:"$(id)", duration:$(duration), value:$(value), max:$(max)}
