...
$execute \
    as @n[sort=arbitrary,tag=cse.status_effect.flying_speed.id.$(id),tag=cse.status_effect.flying_speed.$(type),tag=cse.status_effect.flying_speed.data_target,tag=cse.status_effect.flying_speed.data,distance=0..,type=marker] run \
function cse:status_effects/apply/flying_speed/marker_data/math {duration:$(duration),value:$(value), max:$(max)}

$attribute @s minecraft:flying_speed modifier remove cse.status_effects.flying_speed.$(id).$(type)
...
$data modify storage cse:status_effects flying_speed.final_value.type set value "$(type)"
...
data modify storage temp2 temp set from storage cse:status_effects flying_speed.final_value
