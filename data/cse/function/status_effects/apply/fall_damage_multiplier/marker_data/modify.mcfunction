...
$execute \
    as @n[sort=arbitrary,tag=cse.status_effect.fall_damage_multiplier.id.$(id),tag=cse.status_effect.fall_damage_multiplier.data_target,tag=cse.status_effect.fall_damage_multiplier.data,distance=0..,type=marker] run \
function cse:status_effects/apply/fall_damage_multiplier/marker_data/math {duration:$(duration),value:$(value), max:$(max)}

$attribute @s minecraft:fall_damage_multiplier modifier remove cse.status_effects.fall_damage_multiplier.$(id)
