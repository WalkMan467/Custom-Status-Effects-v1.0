...
$execute \
    if score @s cse.status_effects.fall_damage_multiplier.id = @n[tag=cse.status_effect.fall_damage_multiplier.data,tag=cse.status_effect.fall_damage_multiplier.id.$(id),tag=cse.status_effect.fall_damage_multiplier.$(type),distance=0..,type=marker] cse.status_effects.fall_damage_multiplier.id run \
tag @s remove cse.status_effect.fall_damage_multiplier.new

$attribute @s[tag=cse.status_effect.fall_damage_multiplier.new] minecraft:fall_damage_multiplier modifier add cse.status_effects.fall_damage_multiplier.$(id).$(type) $(base) $(type)

tag @s remove cse.status_effect.fall_damage_multiplier.new

# Detect if there are duplicate IDs
$execute \
    unless entity @n[tag=cse.status_effect.fall_damage_multiplier.data,tag=cse.status_effect.fall_damage_multiplier.id.$(id),tag=cse.status_effect.fall_damage_multiplier.$(type),distance=0..,type=marker] run \
tag @s add cse.status_effect.fall_damage_multiplier.new

# Call function using the result
$execute \
    if entity @s[tag=cse.status_effect.fall_damage_multiplier.new] run \
function cse:status_effects/apply/fall_damage_multiplier/marker_data/add/1 {id:"$(id)", duration:$(duration), base:$(base), value:$(value), max:$(max), type:"$(type)"}
...
$execute \
    ...
    if entity @n[tag=cse.status_effect.fall_damage_multiplier.data,tag=cse.status_effect.fall_damage_multiplier.id.$(id),tag=cse.status_effect.fall_damage_multiplier.$(type),distance=0..,type=marker] run \
function cse:status_effects/apply/fall_damage_multiplier/marker_data/modify {id:"$(id)", duration:$(duration), value:$(value), max:$(max), type:"$(type)"}
