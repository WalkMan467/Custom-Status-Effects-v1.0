...
$execute \
    as @n[sort=arbitrary,tag=cse.status_effect.luck.id.$(id),tag=cse.status_effect.luck.data_target,tag=cse.status_effect.luck.data,distance=0..,type=marker] run \
function cse:status_effects/apply/luck/marker_data/math {duration:$(duration),value:$(value), max:$(max)}

$attribute @s minecraft:luck modifier remove cse.status_effects.luck.$(id)
