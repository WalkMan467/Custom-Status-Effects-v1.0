...
$execute \
    if score @s cse.status_effects.luck.id = @n[tag=cse.status_effect.luck.data,tag=cse.status_effect.luck.id.$(id),distance=0..,type=marker] cse.status_effects.luck.id run \
tag @s remove cse.status_effect.luck.new

$attribute @s[tag=cse.status_effect.luck.new] minecraft:luck modifier add cse.status_effects.luck.$(id) $(base) add_multiplied_base

tag @s remove cse.status_effect.luck.new

# Detect if there are duplicate IDs
$execute \
    unless entity @n[tag=cse.status_effect.luck.data,tag=cse.status_effect.luck.id.$(id),distance=0..,type=marker] run \
tag @s add cse.status_effect.luck.new

# Call function using the result
$execute \
    if entity @s[tag=cse.status_effect.luck.new] run \
function cse:status_effects/apply/luck/marker_data/add/1 {id:"$(id)", duration:$(duration), base:$(base), value:$(value), max:$(max)}
...
$execute \
    ...
    if entity @n[tag=cse.status_effect.luck.data,tag=cse.status_effect.luck.id.$(id),distance=0..,type=marker] run \
function cse:status_effects/apply/luck/marker_data/modify {id:"$(id)", duration:$(duration), value:$(value), max:$(max)}
