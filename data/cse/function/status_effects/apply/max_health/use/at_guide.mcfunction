...
$execute \
    if score @s cse.status_effects.max_health.id = @n[tag=cse.status_effect.max_health.data,tag=cse.status_effect.max_health.id.$(id),distance=0..,type=marker] cse.status_effects.max_health.id run \
tag @s remove cse.status_effect.max_health.new

$attribute @s[tag=cse.status_effect.max_health.new] minecraft:max_health modifier add cse.status_effects.max_health.$(id) $(base) add_multiplied_base

tag @s remove cse.status_effect.max_health.new

# Detect if there are duplicate IDs
$execute \
    unless entity @n[tag=cse.status_effect.max_health.data,tag=cse.status_effect.max_health.id.$(id),distance=0..,type=marker] run \
tag @s add cse.status_effect.max_health.new

# Call function using the result
$execute \
    if entity @s[tag=cse.status_effect.max_health.new] run \
function cse:status_effects/apply/max_health/marker_data/add/1 {id:"$(id)", duration:$(duration), base:$(base), value:$(value), max:$(max)}
...
$execute \
    ...
    if entity @n[tag=cse.status_effect.max_health.data,tag=cse.status_effect.max_health.id.$(id),distance=0..,type=marker] run \
function cse:status_effects/apply/max_health/marker_data/modify {id:"$(id)", duration:$(duration), value:$(value), max:$(max)}
