...
$execute \
    as @n[sort=arbitrary,tag=cse.status_effect.max_health.id.$(id),tag=cse.status_effect.max_health.$(type),tag=cse.status_effect.max_health.data_target,tag=cse.status_effect.max_health.data,distance=0..,type=marker] run \
function cse:status_effects/apply/max_health/marker_data/math {duration:$(duration),value:$(value), max:$(max)}

$attribute @s minecraft:max_health modifier remove cse.status_effects.max_health.$(id).$(type)
...
$data modify storage cse:status_effects max_health.final_value.type set value "$(type)"
...
data modify storage temp2 temp set from storage cse:status_effects max_health.final_value
