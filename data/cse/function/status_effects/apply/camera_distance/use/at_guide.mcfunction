...
$execute \
    if score @s cse.status_effects.camera_distance.id = @n[tag=cse.status_effect.camera_distance.data,tag=cse.status_effect.camera_distance.id.$(id),tag=cse.status_effect.camera_distance.$(type),distance=0..,type=marker] cse.status_effects.camera_distance.id run \
tag @s remove cse.status_effect.camera_distance.new

$attribute @s[tag=cse.status_effect.camera_distance.new] minecraft:camera_distance modifier add cse.status_effects.camera_distance.$(id).$(type) $(base) $(type)

tag @s remove cse.status_effect.camera_distance.new

# Detect if there are duplicate IDs
$execute \
    unless entity @n[tag=cse.status_effect.camera_distance.data,tag=cse.status_effect.camera_distance.id.$(id),tag=cse.status_effect.camera_distance.$(type),distance=0..,type=marker] run \
tag @s add cse.status_effect.camera_distance.new

# Call function using the result
$execute \
    if entity @s[tag=cse.status_effect.camera_distance.new] run \
function cse:status_effects/apply/camera_distance/marker_data/add/1 {id:"$(id)", duration:$(duration), base:$(base), value:$(value), max:$(max), type:"$(type)"}
...
$execute \
    ...
    if entity @n[tag=cse.status_effect.camera_distance.data,tag=cse.status_effect.camera_distance.id.$(id),tag=cse.status_effect.camera_distance.$(type),distance=0..,type=marker] run \
function cse:status_effects/apply/camera_distance/marker_data/modify {id:"$(id)", duration:$(duration), value:$(value), max:$(max), type:"$(type)"}
